...
description: Robust URL-to-Markdown extraction for AI agent workflows. Use when the user wants to "extract/summarize/convert a webpage to markdown" (especially WeChat mp.weixin.qq.com) and web_fetch/browser is blocked or messy. Uses a cheap probe via web_fetch first, then falls back to the official MinerU API (via the local mineru-extract skill) and returns a traceable result contract with source links.
---
...
```bash
uv run python mineru-extract/scripts/mineru_parse_documents.py \
  --file-sources "<URL>" \
