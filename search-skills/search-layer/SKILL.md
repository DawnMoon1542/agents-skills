...
```bash
uv run --with requests python ./search-layer/scripts/search.py \
...
- Exa 端点默认是 `https://api.exa.ai/search`；如需自建/代理，可通过 `EXA_API_BASE`/`EXA_API_URL` 或 `~/.config/search-skills/credentials.json` 里的 `exaApiBase` 覆盖（示例：`https://exa.example.com`）

**Grok 源说明**：
- 通过 completions API 调用 Grok 模型（`grok-4.1-fast`），利用其实时知识返回结构化搜索结果
- 自动检测时间敏感查询并注入当前时间上下文
- 在 deep 模式下与 Exa、Tavily 并行执行
- 需要在 `~/.config/search-skills/credentials.json` 中配置 Grok 的 `apiUrl`、`apiKey`、`model`（或通过环境变量 `GROK_API_URL`、`GROK_API_KEY`、`GROK_MODEL`）
...
```bash
uv run --with requests python search.py "AES-CBC PKCS7 padding bug" --mode deep --intent status --extract-refs
```
...
```bash
uv run --with requests python search.py --extract-refs-urls "https://github.com/owner/repo/issues/123" "https://github.com/owner/repo/issues/456"
```
...
```bash
uv run python fetch_thread.py "https://github.com/owner/repo/issues/123" --format json
uv run python fetch_thread.py "https://github.com/owner/repo/issues/123" --format markdown
uv run python fetch_thread.py "https://github.com/owner/repo/issues/123" --extract-refs-only
```
...
```bash
uv run --with requests python search.py "query" --mode deep --intent tutorial --domain-boost dev.to,freecodecamp.org
```
...
| 场景 | 命令 |
|------|------|
| 快速事实 | `web_search` + `uv run --with requests python search.py --mode answer --intent factual` |
| 深度调研 | `web_search` + `uv run --with requests python search.py --mode deep --intent exploratory` |
| 最新动态 | `web_search(freshness="pw")` + `uv run --with requests python search.py --mode deep --intent status --freshness pw` |
| 对比分析 | `web_search` × 3 queries + `uv run --with requests python search.py --queries "A vs B" "A pros" "B pros" --intent comparison` |
| 找资源 | `web_search` + `uv run --with requests python search.py --mode fast --intent resource` |
