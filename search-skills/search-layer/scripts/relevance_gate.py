...
def _load_creds() -> dict:
    keys = {}
    cred_path = Path.home() / ".config" / "search-skills" / "credentials.json"
...
def score_candidates(
    ...
    try:
        ...
        sys.stderr.write(
            f"[relevance_gate] LLM call failed: {e}, returning all candidates\n"
        )
    ...
    try:
        ...
        sys.stderr.write(
            f"[relevance_gate] Failed to parse LLM response: {raw[:200]}\n"
        )
