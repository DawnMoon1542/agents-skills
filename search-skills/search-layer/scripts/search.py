...
Usage:
  uv run --with requests python search.py "query" --mode deep --num 5
  uv run --with requests python search.py "query" --mode deep --intent status --freshness pw
  uv run --with requests python search.py --queries "q1" "q2" --mode deep --intent comparison
  uv run --with requests python search.py "query" --domain-boost github.com,stackoverflow.com
"""
...
try:
    ...
    print(
        '{"error": "requests library not installed. Run with: uv run --with requests python search.py"}',
        file=sys.stderr,
    )
    sys.exit(1)
...
def score_result(result: dict, query: str, intent: str, boost_domains: set) -> float:
    ...
    score = (
        weights["keyword"] * kw + weights["freshness"] * fr + weights["authority"] * au
    )
...
def _find_credentials() -> str | None:
    ...
    if env_path := os.environ.get("SEARCH_SKILLS_CREDENTIALS"):
        return env_path
    search_skills_dir = Path(__file__).resolve().parents[2]
    candidates = [
        str(search_skills_dir / "credentials.json"),
        os.path.expanduser("~/.config/search-skills/credentials.json"),
...
def get_keys():
    ...
    if cred_path:
        try:
            ...
            if v := (
                cred.get("exaApiUrl")
                or cred.get("exaApiBase")
                or cred.get("exaBaseUrl")
            ):
                keys["exa_url"] = v
...
def search_grok(
    ...
    try:
        ...
        needs_time = any(k in query for k in time_keywords_cn) or any(
            k in query.lower() for k in time_keywords_en
        )
        ...
        if is_sse:
            ...
            choice = choices[0]
            content = (
                (choice.get("message") or {}).get("content") or choice.get("text") or ""
            )
...
def _detect_research_profile(
    query: str, queries: list[str], mode: str, intent: str | None
) -> str | None:
    ...
    if intent == "comparison" and (
        comparison_signal or judgment_signal or query_bundle_signal
    ):
        return "research-light"

    if intent == "exploratory" and (
        judgment_signal or causal_signal or comparison_signal
    ):
        return "research-light"
...
def execute_search(
    ...
    if mode == "fast":
        if "exa" in keys and _want("exa"):
            ...
            all_results = search_grok(
                query, grok_url, grok_key, grok_model, num, freshness
            )
        else:
        ...
        if "tavily" not in keys or not _want("tavily"):
            ...
            tav = search_tavily(
                query, keys["tavily"], num, include_answer=True, freshness=freshness
            )
...
def _run_extract_refs(urls: list) -> list:
    ...
    def _fetch_one(url: str) -> dict:
        try:
            ...
            return {"source_url": url, "refs": [], "ref_count": 0, "error": str(e)}
...
def main():
    ...
    ap.add_argument(
        "--mode",
        choices=["fast", "deep", "answer"],
        default="deep",
        help="fast=Exa only | deep=Exa+Tavily | answer=Tavily with AI answer",
    )
    ap.add_argument(
        "--num", type=int, default=5, help="Results per source per query (default 5)"
    )
    ap.add_argument(
        "--intent",
        choices=[
    ...
    if args.queries:
        ...
        ap.error(
            "Provide a query positional argument, --queries, or --extract-refs-urls"
        )
