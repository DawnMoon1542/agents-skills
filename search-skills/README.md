...
**一组可组合的多源搜索、线程深抓与高保真内容提取的生产级 Skills。**
...
> 本仓库最初为 [github-explorer](https://github.com/blessonism/github-explorer-skill) 构建，后独立为通用搜索能力集合。

---

## 概述

`search-skills` 是一组可组合的搜索能力集合，可集成到任意 AI Agent 运行时中，覆盖从 **找资料**、**抓上下文**、**提正文** 到 **追引用链** 的完整链路。

## 包含什么

| Skill | 干什么的 |
|-------|---------| 
| **[search-layer](./search-layer/)** | 四源并行搜索（Brave + Exa + Tavily + Grok）+ 意图感知评分 + 自动去重 + 链式引用追踪。Brave 通过 Agent 内置的 `web_search` 工具提供，Grok 通过 Completions API 调用。 |
...
```
search-layer ──── Exa + Tavily + Grok 并行搜索 + 意图评分 + 链式追踪
content-extract ── 智能 URL → Markdown
  └── mineru-extract ── MinerU API（反爬场景）
Agent 内置工具 ── web_search (Brave), web_fetch, browser
```
...
- **链式引用追踪**：遇到 GitHub issue / PR、HN、Reddit、V2EX 或网页文章时，可继续沿引用链深挖
...
- Brave（通过 Agent 内置 `web_search` 工具提供）
- Exa
- Tavily
- Grok
...
Exa 在默认主路径中的定位是：**提升基础检索质量的 retrieval source**。
...
- **摘要质量增强**：本地优先使用 `highlights -> text -> summary -> snippet` 构造摘要
...
当普通搜索结果里出现 GitHub issue / PR、论坛帖子、评论线程等"线索节点"时，search-layer 会继续沿引用链深挖。

- **查 GitHub 问题时**：不只看到 issue 标题，还能继续读正文、评论、关联 PR、交叉引用和 commit 线索
- **查社区讨论时**：不只拿到一个 HN / Reddit / V2EX 链接，而是能继续拿到帖子正文与关键回复
- **查网页文章时**：不只保留链接本身，还能提取正文和内部/外部引用
- **做根因分析时**：可以顺着"谁引用了谁、修复落在哪、讨论后来怎么演变"继续往下走
...
Thread-pulling 让你稳定拿到的不是一个链接，而是一份可继续分析、可继续追踪的上下文材料：
...
将本仓库 clone 到本地，然后将三个 skill 目录链接到你的 Agent 运行时 skills 目录：

```bash
# 1. Clone 到本地
git clone https://github.com/blessonism/search-skills.git ~/search-skills

# 2. 链接到你的 skills 目录（路径取决于你的 Agent 运行时）
...
> 不同的 Agent 运行时，skills 目录位置不同。请参考你所使用的 Agent 运行时的文档。
...
创建 `~/.config/search-skills/credentials.json`：
...
> 也可通过环境变量 `SEARCH_SKILLS_CREDENTIALS` 指定凭证文件路径。

**方式二：环境变量（兼容）**

```bash
export EXA_API_KEY="your-exa-key"        # https://exa.ai
# 可选：自定义 Exa 端点（用于自建/代理 Exa）
...
- 凭证文件：在 `~/.config/search-skills/credentials.json` 里增加 `"exaApiBase": "https://exa.example.com"`

（也支持把 `exa` 写成对象：`{"exa": {"apiKey": "...", "apiUrl": "https://exa.example.com"}}`）

Brave API Key 由 Agent 运行时的内置 `web_search` 工具管理，不需要在这里配置。
...
MinerU 解析结果默认输出到 `~/.cache/search-skills/mineru/`。可通过环境变量 `SEARCH_SKILLS_WORKSPACE` 自定义：
...
- **源过滤**：`--source grok,exa` 指定只使用特定搜索源
- **默认模型升级**：Grok 默认模型从 `grok-4.1` 切换到 `grok-4.1-fast`
- **Thinking 标签处理**：自动剥离 Grok thinking 模型的 `<think>` 标签
- **JSON 提取增强**：处理 Grok 在 JSON 前输出自然语言文字的情况
- **凭证文件**：统一凭据管理，`~/.config/search-skills/credentials.json` 集中存放所有搜索源 key

### search-layer v2.1 特性

v2.1 新增 **Grok (xAI)** 作为第四搜索源，通过 Completions API 调用，支持 API 代理站：

- **Grok 搜索源**：利用 Grok 模型的实时知识返回结构化搜索结果
- **四源并行**：Deep 模式下 Exa + Tavily + Grok 三源并行
- **智能降级**：Grok 配置缺失时自动降级为 Exa + Tavily 双源
