...
Notes
- This is NOT an MCP server. It's a script meant to be called by AI agent skills via exec.
...
- SEARCH_SKILLS_WORKSPACE (optional): workspace/cache root, default ~/.cache/search-skills
...
def _default_workspace() -> pathlib.Path:
    ...
    if v := os.environ.get("SEARCH_SKILLS_WORKSPACE"):
        return pathlib.Path(v)
    return pathlib.Path.home() / ".cache" / "search-skills"
...
def poll_task(
    *, api_base: str, token: str, task_id: str, timeout_sec: int, poll_interval: float
) -> dict:
    ...
    while True:
        res = _http_json(
            "GET", endpoint, headers={"Authorization": f"Bearer {token}"}, timeout=60
        )
        if res.get("code") != 0:
        ...
        if state == "failed":
            raise RuntimeError(
                f"MinerU task failed: {data.get('err_msg') or '(no err_msg)'}"
            )
        if time.time() - start > timeout_sec:
            raise RuntimeError(
                f"MinerU poll timeout after {timeout_sec}s (last state={state})"
            )
        time.sleep(poll_interval)


def extract_main_markdown(
    zip_bytes: bytes, out_dir: pathlib.Path
) -> pathlib.Path | None:
...
def main() -> int:
    ...
    ap.add_argument(
        "--file-sources",
        required=True,
        help="Comma/newline separated URLs or local paths (MCP-style).",
    )
    ap.add_argument(
        "--enable-ocr", action="store_true", help="Enable OCR (maps to MinerU is_ocr)."
    )
    ap.add_argument("--language", default="ch")
    ap.add_argument("--page-ranges", default=None)
    ap.add_argument(
        "--model-version", default=None, help="pipeline | vlm | MinerU-HTML"
    )
    ...
    ap.add_argument(
        "--extra-formats", default=None, help="Comma-separated: docx,html,latex"
    )
    ...
    ap.add_argument(
        "--emit-markdown",
        action="store_true",
        help="Include markdown text in JSON (can be large).",
    )
    ap.add_argument(
        "--max-chars",
        type=int,
        default=20000,
        help="When --emit-markdown, truncate markdown to this many chars.",
    )
