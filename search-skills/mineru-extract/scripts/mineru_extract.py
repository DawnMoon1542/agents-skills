...
Designed to be called from AI agent skills via `exec`.
...
- SEARCH_SKILLS_WORKSPACE: workspace root for output (default: ~/.cache/search-skills)
"""
...
def main() -> int:
    ...
    ap.add_argument(
        "--api-base", default=os.environ.get("MINERU_API_BASE", "https://mineru.net")
    )
    ap.add_argument("--timeout", type=int, default=600)
    ap.add_argument("--poll-interval", type=float, default=2.0)

    ap.add_argument(
        "--ocr", dest="is_ocr", action=argparse.BooleanOptionalAction, default=None
    )
    ap.add_argument(
        "--formula",
        dest="enable_formula",
    ...
    ap.add_argument(
        "--extra-formats",
        default=None,
        help="Comma-separated: docx,html,latex (md+json are default).",
    )

    ap.add_argument(
        "--out",
        default=None,
        help="Output directory (default: workspace/mineru/<task_id>).",
    )
    ap.add_argument(
        "--print",
        dest="do_print",
        action="store_true",
        help="Print extracted markdown to stdout.",
    )
    ap.add_argument(
        "--max-chars", type=int, default=12000, help="When --print, max chars to print."
    )
    ...
    base_out = (
        ...
        if args.out
        else (workspace / "mineru" / sanitize_filename(task_id))
    )
